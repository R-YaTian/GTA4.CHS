...
#if PLUGIN_USE_FREETYPE
void CPlugin::LoadFont()
{
    char str[255];

    if (GetPrivateProfileStringA("Config", "Font", "", str, 255, GetPluginAsset("config.ini").string().c_str()) > 0)
    {
        plugin.renderer.LoadFont(GetPluginAsset(str));
    }
}
#endif
